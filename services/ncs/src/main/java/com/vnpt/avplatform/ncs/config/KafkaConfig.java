...
/**
 * Kafka configuration for the NCS service.
 *
 ...
 * {@code <original-topic>.DLT} (FR-NCS-031).</p>
 */
@Slf4j
@Configuration
public class KafkaConfig {
    ...
    /**
     * KafkaTemplate with TenantIdPartitioner configured so all NCS events for the
     ...
     * an infinite consumer loop (FR-NCS-031).</p>
     *
    ...
    private CommonErrorHandler deadLetterErrorHandler() {
        ...
        handler.setRetryListeners((record, ex, deliveryAttempt) ->
                log.warn("NCS Kafka retry attempt={} for topic={} partition={} offset={}: {}",
                        deliveryAttempt,
                        record.topic(),
                        record.partition(),
                        record.offset(),
