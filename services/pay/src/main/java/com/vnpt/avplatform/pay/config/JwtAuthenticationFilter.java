...
/**
 * JWT authentication filter for the PAY service.
 *
 * <p>Validates Bearer tokens on every incoming request. In the PAY service context,
 * valid tokens must carry {@code ROLE_INTERNAL_SERVICE} or {@code ROLE_PLATFORM_ADMIN}
 * in their {@code roles} claim for authorization to proceed.</p>
 */
@Slf4j
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (token != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                log.debug("PAY JWT authentication successful for subject: {}", claims.getSubject());
            } catch (ExpiredJwtException e) {
                log.warn("PAY: JWT token expired for request [{}]: {}", request.getRequestURI(), e.getMessage());
            } catch (JwtException e) {
                log.warn("PAY: invalid JWT token for request [{}]: {}", request.getRequestURI(), e.getMessage());
            }
        }
