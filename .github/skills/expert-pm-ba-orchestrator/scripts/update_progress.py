...
def update_file_progress(file_path, keywords, status="x", cascade=False):
    ...
    for line in lines:
        ...
        # Section Detection logic for Master Plan cascading
        # If we find a header with the keyword, enter "target section" mode
        # If we find a new header at same level, exit mode
        if cascade and not force_all:
             # Check for Header Lines like "### 6.1 WP-040:"
             ...
             if line.strip().startswith("#"):
                ...
                if header_match:
                    ...
                    print(f"   🌊 Entered Target Section in {os.path.basename(file_path)}: {line.strip()}")
                elif in_target_section:
                    # Only exit if we hit a new Header that looks like a WP header
                    # Check if line starts with # and has WP- pattern
                    if line.strip().startswith("#") and "WP-" in line:
                         # It is a WP header, but is it ours?
                         # We already checked header_match above. If we are here, header_match is False.
                         # So it's a different WP header. Exit section.
                         in_target_section = False
                         # print(f"   🔚 Exited Target Section: {line.strip()}")
        ...
        if match_found and "- [" in line:
            ...
            # Debug
            # if in_target_section:
            #    print(f"DEBUG: Processing '{line.strip()}' -> '{new_line.strip()}' | Match: {match_found}")
...
def main():
    ...
    parser.add_argument("keyword", help="Keyword or Task ID (e.g., TASK-001 or WP-001)")
    ...
    keywords_to_update = [args.keyword]
    
    # Hierarchy Resolution: Find parent WP
    parent_wp = get_parent_wp(args.master, args.keyword)
    if parent_wp and parent_wp not in keywords_to_update:
        print(f"ℹ️  Detected Parent Work Package: {parent_wp}")
        if args.cascade:
    ...
    # Pass cascade=args.cascade to allow section-based updating if requested
    master_updated = update_file_progress(args.master, keywords_to_update, args.status, cascade=args.cascade)
